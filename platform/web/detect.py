...
def get_opts():
    ...
    return [
        ...
        # Matches default values from before Emscripten 3.1.27. New defaults are too low for Test Game Engine.
...
def configure(env: "SConsEnvironment"):
    ...
    if cc_semver < (4, 0, 0):
        print_error("The minimum Emscripten version to build Test Game Engine is 4.0.0, detected: %s.%s.%s" % cc_semver)
        sys.exit(255)
    ...
    # Wrap the JavaScript support code around a closure named Test Game Engine.
    env.Append(LINKFLAGS=["-sMODULARIZE=1", "-sEXPORT_NAME='Test Game Engine'"])
