...
if [ ! -f "version.py" ]; then
  echo "Warning: This script is intended to be run from the root of the Test Game Engine repository."
  echo "Some of the paths checks may not work as intended from a different folder."
fi

if [ $# != 1 ]; then
  echo "Usage: @0 <path-to-test game engine-executable>"
  exit 1
fi
...
while read -r dir; do
    ...
    wget -nv --retry-on-http-error=503 --tries=5 --timeout=60 -cO "$reference_file" "https://raw.githubusercontent.com/godotengine/test game engine-headers/test game engine-$reference_tag/extension_api.json" || has_problems=1
