...
1. Build Test Game Engine with the module enabled: `module_mono_enabled=yes`.
2. After building Test Game Engine, use it to generate the C# glue code:
   ```sh
   <godot_binary> --generate-mono-glue ./modules/mono/glue
   ```
3. Build the C# solutions:
   ```sh
   ./modules/mono/build_scripts/build_assemblies.py --test game engine-output-dir ./bin
   ```

The paths specified in these examples assume the command is being run from
the Test Game Engine source root.
...
In order to use Test Game Engine with a development of those packages, we must create
...
The Test Game Engine NuGet packages must be added to that local source. Additionally,
...
Follow the above instructions but build Test Game Engine with the precision=double argument to scons
