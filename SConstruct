...
opts.Add(BoolVariable("production", "Set defaults to build Test Game Engine for use in production", False))
...
opts.Add("vsproj_name", "Name of the Visual Studio solution", "test game engine")
...
if env["platform"] in compatibility_platform_aliases:
    ...
    print_warning(
        f'Platform "{alias}" has been renamed to "{platform}" in Test Game Engine 4. Building for platform "{platform}".'
    )
...
if cc_version_major == -1:
    ...
    if cc_version_major < 9:
        print_error(
            ...
            "C++17, or has bugs when compiling Test Game Engine. Supported versions are 9 "
    ...
    # https://github.com/godotengine/test game engine/pull/94995#issuecomment-2336464574
    if cc_version_major == 16 and cc_version_minor < 11:
        print_error(
            "Detected Visual Studio 2019 version older than 16.11, which has bugs "
            "when compiling Test Game Engine. Use a newer VS2019 version, or VS2022."
        )
        Exit(255)
    if cc_version_major == 15 and cc_version_minor < 9:
        print_error(
            "Detected Visual Studio 2017 version older than 15.9, which has bugs "
            "when compiling Test Game Engine. Use a newer VS2017 version, or VS2019/VS2022."
        )
        Exit(255)
    if cc_version_major < 15:
        print_error(
            "Detected Visual Studio 2015 or earlier, which is unsupported in Test Game Engine. "
...
# Disable exception handling. Test Game Engine doesn't use exceptions anywhere, and this
