...
The `grisu2.h` file is the same as `to_chars.cpp` but with `test game engine.patch` applied to it, with the following changes:
...
- Remove the trailing `.0` logic to match Test Game Engine's existing `String::num_scientific` behavior.
